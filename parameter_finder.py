...
def GAMMA_CHECK(gamma,  n, calF, theta=np.linspace(-np.pi,np.pi, 100), tol=10**(-16), show='fcns',  alpha=1, ax=None, **plt_kwargs):
    """
...
def PARAMETER_FIND(czlist, szlist,n, data,epsi=10**(-14), defconv='ad', ifdecomp=True, tDict={}, plots=False, axeschoice=[0, 1]):
    ...
    #GAMMA_CHECK(gammaW,  n, calF, theta=data, tol=10**(-14), show='diff',  alpha=1, ax=axeschoice[0])
    ...
    if probflag==1:
        ...
        gammaW, itW, initgamma, nu=wm.WILSON_LOOP_WDATA(calFc.astype('complex'), 2*n,  nu=epsifejer, init="Wilson2", datatype='complex')
        c, d, probflag=cd_PROCESS(gammaW, a, b,n, tol=epsi, plots=plots, ax=axeschoice[1])
        t1=time.perf_counter()
